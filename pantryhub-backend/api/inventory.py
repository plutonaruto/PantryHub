...
@inventory_bp.route('/items/owner/<string:owner_id>', methods=['GET'])
@login_required
def get_all_items(owner_id):
    try:
        print("Fetching items for owner_id:", owner_id)
        items = db.session.query(Item).filter_by(owner_id=owner_id).all()
        result = []
        for item in items:
            result.append({
                "id": item.id,
        ...
        print("Error in GET /items/owner/<owner_id>:", str(e))
